...
public class IntroUI extends JFrame {
	...
	private void loadConfig() {
		try {
			applicationProperties = new PropertiesConfiguration("application.properties");
			versionProperties = new PropertiesConfiguration("version.properties");
	...
	public IntroUI() {
		...
		JLabel teamNameDetails = new JLabel(applicationProperties.getString("TeamName"));
		contentPane.add(teamNameDetails, "4, 2");

		JLabel lblTeamLeader = new JLabel("Team leader:");
		contentPane.add(lblTeamLeader, "2, 4");

		JLabel teamLeaderDetails = new JLabel(applicationProperties.getString("TeamLeader"));
		...
		JLabel teamLeaderEmailDetails = new JLabel(applicationProperties.getString("TeamLeaderEmail"));
		...
		try {
		    img = ImageIO.read(new File(applicationProperties.getString("TeamLogo")));
		} catch (IOException e) {
		    e.printStackTrace();
		}
		ImageIcon imageIcon = new ImageIcon(img.getScaledInstance(200, 100, Image.SCALE_SMOOTH));
		...
		JLabel versionDetails = new JLabel(versionProperties.getString("build.number"));
